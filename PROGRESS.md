...
## Home Restructure: Category List + Favorites + Splash (done, verified live)
- [x] Home rebuilt from filter-chips + grid to a vertical list of the 20 categories, each with a real Wiro-generated landscape cover image (4:3, one-off `seedCategoryPreviews` admin function — same deploy/curl-300x/delete pattern as theme previews, 20/20 succeeded, 0 failures). Tapping a category pushes `CategoryStylesViewController`, a 2-column grid of that category's styles (reuses `ThemeCardCell`). Verified on simulator via a `NS_DEBUG_CATEGORY` debug hook + screenshots — real cover images and real style images both load correctly.
- [x] Loading spinners added to every async-image cell: `CategoryListCell` (Home), `ThemeCardCell` (style grids), and `GalleryCell` (Gallery history) all show a centered `UIActivityIndicatorView` until the image finishes loading.
- [x] Favorites: new `users/{uid}/favorites/{styleId}` Firestore subcollection (rules deployed), `FavoritesService` (fetch ids, fetch full `ThemeCard`s via an `in` query, toggle). Tapping the heart on any `ThemeCardCell` (in `CategoryStylesViewController` or Gallery's Favorites tab) toggles it with an optimistic local update + haptic. Verified end-to-end via a `NS_DEBUG_AUTO_FAVORITE_FIRST` debug hook: favorited from the category screen, confirmed it appears (filled heart) in Gallery's Favorites segment.
- [x] Gallery tab: added a "History / Favorites" `UISegmentedControl` at the top. History shows the existing `generations` grid; Favorites shows the user's favorited styles (tapping one starts a new generation from that style). Contextual empty states per segment. Verified on simulator (both segments render correctly, including the populated Favorites case).
- [x] Splash screen: new `SplashViewController` — animated pink logo badge (spring scale-in), pulsing glow ring, fading title/subtitle, bouncing loading dots — shown by `AppCoordinator` before the onboarding/paywall/home routing decision (~1.6s, then crossfades into the existing flow). Native `UILaunchScreen` background color changed from unset to a new `SplashBackground` color asset matching the splash's cream background exactly, so there's no flash/jump between the OS launch screen and the animated splash. Verified via a `NS_DEBUG_SCREEN=splash` hook + screenshots showing the full animated state.
- [x] `GenerationService`/`generateContent` simplified: the user's source photo is now sent to Wiro as a real multipart file attachment directly from the Cloud Function, no Firebase Storage upload round-trip. Verified empirically that Wiro's undocumented multipart support actually uses the attached file (not just ignoring it) by adding a distinguishing prop to a reference photo and confirming it appeared in the output; then verified end-to-end via curl (200 OK, correct credit spend, real image generated, no Storage write for the source image). `storage.rules`' now-unused `uploads/**` rule removed.
