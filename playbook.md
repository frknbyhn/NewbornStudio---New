...
| **[native Swift/UIKit] XcodeGen `resources:` target key silently wires nothing** — `xcodegen generate` prints "Created project", `xcodebuild build` says **BUILD SUCCEEDED**, but fonts fall back to system font and `GoogleService-Info.plist` is just missing from the `.app` (crashes at runtime with a Firebase-config error that looks unrelated) | Verified on XcodeGen 2.46.0 / Xcode 26.2 (`objectVersion 77`): the `resources:` key never emits a `PBXResourcesBuildPhase`, reproducible even in a bare minimal project with a single `Assets.xcassets`. **Fix:** fold resource paths into `sources:` instead, each with `buildPhase: resources` — that produces a real resources build phase and the files actually land in the built app. **Detect:** after every `xcodegen generate`, `grep -c PBXResourcesBuildPhase *.xcodeproj/project.pbxproj` — `0` means nothing under `resources:` was wired, regardless of what the CLI printed. |
| **Cloud Functions v2 `getSignedUrl()` → `SigningError: Permission 'iam.serviceAccounts.signBlob' denied`** at runtime, not deploy time | The default compute service account can read/write Storage objects but not sign a URL with its own key — that needs a separate `roles/iam.serviceAccountTokenCreator` grant on itself, which needs `gcloud`-authenticated IAM access you may not have (only `firebase login` was set up here). **Fix: skip signed URLs, use Firebase's own download-token scheme instead** — `file.setMetadata({metadata:{firebaseStorageDownloadTokens: crypto.randomUUID()}})`, then build `https://firebasestorage.googleapis.com/v0/b/{bucket}/o/{encodeURIComponent(path)}?alt=media&token={token}`. Plain HTTPS GET, works for any external fetcher (verified feeding it to a third-party image API as the input-image URL), no IAM grant needed since `setMetadata` is a normal object write. |
| **A shell script that sets `UID=$(...)` throws `failed to change user ID: operation not permitted`** — no reference to users/permissions anywhere in the script | `UID` is a read-only special shell variable (the real user ID) in bash/zsh; assigning to it is interpreted as trying to change the process's user id. **Fix:** name the variable anything else (`TEST_UID`, `MY_UID`, ...). **Detect:** if a `NAME=$(...)` assignment throws a user-ID error, check whether `NAME` collides with a shell-reserved variable (`UID`, `EUID`, `PPID`, `RANDOM`, ...) before looking anywhere else. |
| **RevenueCat: setting an offering's `is_current: true` silently breaks a DIFFERENT app's paywall** — if the RevenueCat account hosts multiple apps in one shared project (common for an indie dev with several apps under one company account) | `is_current` is a **project-wide** flag, not per-app — despite every other v2 resource (apps, products, entitlements) being app-scoped, offerings have no `app_id` at all, and only one offering in the whole project can be current. Setting it for app A silently un-sets whichever offering app B had. **Fix: never set `is_current` in a shared-project account.** Fetch the offering by its own identifier instead — `offerings.offering(identifier: "...")` — which needs no project-wide flag and can't touch any other app. If already flipped, `POST` the original offering with `is_current: true` to restore it immediately. **Detect:** before ever touching `is_current`, `GET /v2/projects/{id}/offerings` and check which offering currently holds it — if it's not yours, stop. |
| **A re-seed script wipes fields added to a document AFTER the original seed** (e.g. a `previewImageUrl` set by a later batch job vanishes when you re-run the seeder to add an unrelated field) | Admin SDK `doc.set(data)` **replaces the whole document**, not just the fields you pass — the classic Firestore footgun. Re-running a "seed the catalog" script to add one new field (here: a `categories` collection alongside `ai_models`) also re-set every `ai_models` doc with only its original seed fields, deleting `previewImageUrl` that a completely separate later script had added. Caught immediately because Home went back to solid-color placeholder cards. **Fix: `doc.set(data, {merge: true})`** on any seed/admin script that might run again after other writes have landed on the same documents — merge is nearly always what a re-seed actually wants. **Recovery, if it already happened:** if another process's output is still recoverable (the actual files were untouched in this case, only the Firestore field pointing to them was wiped), replay just that field from whatever local record exists (a progress log, a script's own output) via a small merge-only admin write — no need to redo the expensive original work. **Detect:** any time a "just add one field" admin script touches documents another process also writes to, ask whether it uses `.set()` without merge before running it. |
